...
class Root(object):
    ...
    def rating(self, **bechdel_response):
        ...
        if bechdel['rating'] == 3 and bechdel['dubious'] == 0:
            ...
            higher_query = "select imdb_id, title, bechdel_rating, imdb_rating, tomato_meter, genre from movies where bechdel_rating > {0} and imdb_rating > {1} and tomato_meter > {2} and (id = -1 {3}) order by bechdel_rating desc, imdb_rating desc, tomato_meter desc limit 25".format(bechdel['rating'], omdb_response['imdbRating'], omdb_response['tomatoMeter'] if omdb_response['tomatoMeter'] != 'N/A' else 0, ''.join([" or genre like '%{0}%'".format(x) for x in omdb_response['Genre'].split(', ')]))
